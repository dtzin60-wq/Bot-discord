...
def init_db():
    ...
    db_exec("CREATE TABLE IF NOT EXISTS config (chave TEXT PRIMARY KEY, valor TEXT)")
    db_exec("CREATE TABLE IF NOT EXISTS pix_saldo (user_id INTEGER PRIMARY KEY, saldo REAL DEFAULT 0.0)")
...
def reg_stats(u, win):
    ...
    if win: db_exec("UPDATE perfis SET vitorias=vitorias+1, consecutivas=consecutivas+1, total_partidas=total_partidas+1, coins=coins+1 WHERE user_id=?", (u,))
    else: db_exec("UPDATE perfis SET derrotas=derrotas+1, consecutivas=0, total_partidas=total_partidas+1 WHERE user_id=?", (u,))
...
class ViewConfirmacao(View):
    ...
    async def conf(self, it, b):
        ...
        if len(self.cnf) >= len(self.jog):
            self.stop()
            mu = self.modo.upper(); p = "Sala"; t = "geral"
            if "MOBILE" in mu: p, t = "Mobile", "mobile"
            elif "MISTO" in mu: p, t = "Misto", "misto"
            elif "FULL" in mu: p, t = "Full", "full"
            elif "EMU" in mu: p, t = "Emu", "emu"
            
            try: await it.channel.edit(name=f"{p}-{inc_counter(t)}")
            except: pass
            
            e = discord.Embed(title="Partida Confirmada", color=COR_VERDE); e.set_thumbnail(url=BONECA)
            e.add_field(name="🎮 Estilo de Jogo", value=f"{self.modo.split('|')[0]} Gel Normal", inline=False)
            try: tx = f"R$ {max(float(self.val.replace('R$','').replace(',','.').strip())*0.1, 0.1):.2f}".replace('.',',')
            except: tx = "R$ 0,10"
            e.add_field(name="ℹ️ Informações da Aposta", value=f"Valor Da Sala: {tx}\nMediador: <@{self.med}>", inline=False)
            e.add_field(name="💎 Valor da Aposta", value=f"R$ {self.val}", inline=False)
    ...
    async def reg(self, it, b): await it.response.send_message(f"🏳️ {it.user.mention} sugeriu regras.", ephemeral=False)
...
class ViewFila(View):
    ...
    def emb(self):
        e=discord.Embed(title=f"Aposta | {self.m.replace('|',' ')}", color=COR_EMBED); e.set_author(name="WS APOSTAS", icon_url=ICONE)
        e.add_field(name="📋 Modalidade", value=f"**{self.m.replace('|',' ')}**", inline=True)
        e.add_field(name="💰 Valor", value=f"**R$ {self.v}**", inline=True)
    ...
    async def join(self, it, t):
        ...
        if len(self.j) >= lim:
            ...
            cf = get_conf("canal_th")
            if not cf: return await it.channel.send("❌ Configure /canal")
            
            ch = bot.get_channel(int(cf))
            th = await ch.create_thread(name="aguardando-confirmacao", type=discord.ChannelType.public_thread)
            ew = discord.Embed(title="Aguardando Confirmações", color=COR_VERDE); ew.set_thumbnail(url=BONECA)
            ew.add_field(name="👑 Modo:", value=f"{self.m.split('|')[0]} | {self.j[0]['t'] or 'Padrão'}", inline=False)
            ew.add_field(name="💎 Valor:", value=f"R$ {self.v}", inline=False)
            ew.add_field(name="⚡ Jogadores:", value="\n".join([x['m'] for x in self.j]), inline=False)
            ...
            await th.send(content=" ".join([x['m'] for x in self.j]), embed=ew, view=ViewConfirmacao(self.j, md, self.v, self.m))
            self.j = []; await it.message.edit(embed=self.emb())

    async def leave(self, it):
        self.j=[x for x in self.j if x['id']!=it.user.id]; await it.response.edit_message(embed=self.emb())
...
class ViewConfig(View):
    ...
    async def bf(self, it, b): await it.response.send_message("Use o comando .fila para gerar", ephemeral=True)

# === COMANDOS SLASH ===
@bot.tree.command(name="perfil", description="Ver suas estatísticas")
async def slash_perfil(it: discord.Interaction, usuario: discord.Member = None):
    await it.response.defer(); alvo = usuario if usuario else it.user
    d = db_query("SELECT vitorias, derrotas, consecutivas, total_partidas, coins FROM perfis WHERE user_id=?", (alvo.id,))
    v, dt, c, t, co = d if d else (0, 0, 0, 0, 0)
    e = discord.Embed(color=COR_VERDE); e.set_author(name=alvo.name, icon_url=alvo.display_avatar.url)
    e.add_field(name="🎮 Estatísticas", value=f"Vitórias: {v}\nDerrotas: {dt}\nConsecutivas: {c}\nTotal de Partidas: {t}", inline=False)
...
async def slash_gg(it: discord.Interaction, vencedor: discord.Member, perdedor: discord.Member):
    if not it.user.guild_permissions.manage_messages: return await it.response.send_message("❌ Sem permissão.", ephemeral=True)
    reg_stats(vencedor.id, True); reg_stats(perdedor.id, False)
    await it.response.send_message(embed=discord.Embed(title="✅ Resultado Registrado", description=f"🏆 **Vencedor:** {vencedor.mention}\n💀 **Perdedor:** {perdedor.mention}", color=COR_VERDE))
...
async def slash_pix(it: discord.Interaction):
    ...
    e=discord.Embed(title="Painel Para Configurar Chave PIX", color=COR_EMBED); e.set_thumbnail(url=ICONE)
...
async def slash_botconfig(it: discord.Interaction):
    ...
    await it.response.defer(ephemeral=False); e=discord.Embed(title="Painel Config", color=COR_EMBED)
    e.set_thumbnail(url=ICONE); e.description="**Painel Geral**"; await it.followup.send(embed=e, view=ViewConfig())
...
async def mediar(ctx):
    ...
    class VM(View):
        def e(self): return discord.Embed(title="Painel da fila controladora", description="**Entre na fila para começar a mediar suas filas**\n\n"+"\n".join([f"• <@{u}>" for u in fila_mediadores]), color=COR_EMBED)
        ...
        async def rm(self,i,b):
            v=View(); s=UserSelect(placeholder="Selecione o mediador para remover")
            async def cb(ii): 
            ...
            s.callback=cb; v.add_item(s); await i.response.send_message("Quem remover?", view=v, ephemeral=True)
        @discord.ui.button(label="Painel Staff", style=discord.ButtonStyle.secondary, emoji="⚙️")
        async def st(self,i,b): await i.response.send_message("Staff Log", ephemeral=True)
...
async def fila(ctx):
    ...
    class MF(Modal, title="Gerar Filas"):
        m=TextInput(label="Modo", default="1v1"); p=TextInput(label="Plataforma", default="Mobile")
        v=TextInput(label="Valores (Separe por ESPAÇO)", default="100,00 50,00 20,00 10,00 5,00")
        async def on_submit(self, i):
            await i.response.send_message("Gerando filas...", ephemeral=True)
            raw_vals = self.v.value.split() 
            vals = [v.strip()+(",00" if "," not in v else "") for v in raw_vals][:15]
            for val in vals: await i.channel.send(embed=ViewFila(f"{self.m.value}|{self.p.value}", val).emb(), view=ViewFila(f"{self.m.value}|{self.p.value}", val)); await asyncio.sleep(0.5)
    
    await ctx.send("Painel Admin", view=View().add_item(Button(label="Gerar", style=discord.ButtonStyle.danger, custom_id="g")))
    class Btn(View):
        @discord.ui.button(label="Gerar", style=discord.ButtonStyle.danger)
        async def g(self, i, b): await i.response.send_modal(MF())
    await ctx.send("Clique abaixo:", view=Btn())
