...
class ViewAuxiliar(View):
    ...
    async def vitoria(self, it, btn):
        await it.response.send_message("🏆 Vitória declarada pelo mediador! Partida encerrada.")
    ...
    async def wo(self, it, btn):
        await it.response.send_message("⚠️ Vitória por W.O declarada.")
        await self.thread.edit(locked=True, archived=True)
...
class ViewPixMediador(View):
    ...
    async def enviar(self, interaction: discord.Interaction, button: Button):
        ...
        if not cargo_id or not any(r.id == int(cargo_id) for r in interaction.user.roles):
            return await interaction.response.send_message("❌ Apenas mediadores podem enviar o Pix.", ephemeral=True)
        ...
        if not res: return await interaction.response.send_message("❌ Cadastre seu pix com `.pix`.", ephemeral=True)
...
class ViewConfirmarPartida(View):
    ...
    async def btn_confirmar(self, interaction: discord.Interaction, button: Button):
        ...
        if len(set(dados["confirmados"])) >= 2:
            embed_ready = discord.Embed(title="💳 AGUARDANDO PAGAMENTO", color=0xf1c40f, description="Ambos confirmaram! Mediador, envie o Pix abaixo.")
    ...
    async def btn_recusar(self, interaction: discord.Interaction, button: Button):
        await interaction.response.send_message(f"❌ {interaction.user.mention} recusou. Tópico fechado.")
    ...
    async def btn_regras(self, interaction: discord.Interaction, button: Button):
        await interaction.response.send_message("📝 Combinem as regras antes de pagar!", ephemeral=True)
...
class ViewFilaPartida(View):
    ...
    async def entrar(self, interaction, submodo):
        ...
        if len(match) >= 2:
            ...
            mediador_at = fila_atendimento[0].mention if fila_atendimento else "*Ninguém na fila*"
            emb = discord.Embed(title="Aguardando Confirmações", color=0x2ecc71)
            emb.add_field(name="👮 Mediador:", value=mediador_at, inline=False)
            ...
            await thread.send(content=f"{p1.mention} {p2.mention} {fila_atendimento[0].mention if fila_atendimento else ''}", embed=emb, view=ViewConfirmarPartida(thread.id))
...
async def botconfig(ctx):
    class ConfigV(View):
        ...
        async def s_mediador(self, it, sel):
            ...
            await it.response.send_message(f"✅ Cargo {sel.values[0].mention} para Pix salvo!", ephemeral=True)
...
async def aux(ctx):
    ...
    if not isinstance(ctx.channel, discord.Thread):
        return await ctx.send("❌ Use dentro do tópico.")
...
async def mediar(ctx):
    class ViewSuporte(View):
        ...
        async def e(self, it, b):
            ...
            await it.response.defer(); await self.atualizar(it)
...
async def fila(ctx, v: str):
    try:
        ...
        emb.add_field(name="Valor", value=f"R$ {formatar_real(val)}", inline=False)
...
async def pix(ctx):
    class PM(Modal, title="Meu Pix"):
        ...
        q = TextInput(label="Foto do QR Code (Link)")
