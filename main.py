...
# ================= SISTEMA DE PIX (VISUAL ORIGINAL) =================
...
class VPix(View):
    ...
    async def ver_sua(self, it: discord.Interaction, b):
        ...
        if not r: return await it.followup.send("❌ Não tens chave cadastrada.", ephemeral=True)
...
# ================= PAINEL DE MEDIADORES (LISTA NUMERADA) =================
class VMed(View):
    def __init__(self):
        super().__init__(timeout=None)
...
# ================= TÓPICO (LIMPEZA E ENVIO AUTO PIX) =================
class ViewTopico(View):
    ...
    async def c(self, it: discord.Interaction, b):
        if it.user.id not in [self.p1, self.p2]: return await it.response.send_message("❌ Não participas nesta aposta.", ephemeral=True)
        if it.user.id in self.conf: return await it.response.send_message("⚠️ Já confirmaste.", ephemeral=True)
        
        await it.response.defer()
        self.conf.add(it.user.id)
        await it.followup.send(embed=discord.Embed(title="✅ | Confirmação", description=f"<@{it.user.id}> confirmou a partida!", color=0x2ecc71))
        
        if len(self.conf) == 2:
            await asyncio.sleep(1.5)
            await it.channel.purge(limit=25) # Limpa o chat para enviar o PIX
            ...
            emb_p.set_footer(text="Envia o comprovante abaixo para receber os dados da sala.")
    ...
    async def r(self, it: discord.Interaction, b):
        ...
        await it.response.send_message(f"❌ Partida cancelada. O tópico será deletado.")
    ...
    async def rules(self, it: discord.Interaction, b):
        await it.response.send_message("📝 Combinem as regras adicionais aqui no chat.", ephemeral=True)
...
class ViewFila(View):
    ...
    async def entrar(self, it, gelo):
        ...
        if len(self.users) == 2:
            ...
            if not fila_mediadores: return await it.followup.send("❌ Sem mediadores!", ephemeral=True)
            med_id = fila_mediadores.pop(0)
            c_id = pegar_config("canal_1"); canal = bot.get_channel(int(c_id)) if c_id else it.channel
            th = await canal.create_thread(name="confirmar-partida", type=discord.ChannelType.public_thread)
            partidas_ativas[th.id] = {'modo': f"{self.modo} | {gelo}", 'valor': self.valor, 'p1': p1.id, 'p2': p2.id, 'med': med_id}
            
            emb_m = discord.Embed(title="Aguardando Confirmações", color=0x2b2d31)
            emb_m.description = f"**👑 Modo:**\n{self.modo} | {gelo}\n\n**💸 Valor da aposta:**\nR$ {self.valor}\n\n**👮 Mediador:**\n<@{med_id}>\n\n**✨ Jogadores:**\n<@{p1.id}>\n<@{p2.id}>"
            await th.send(content=f"<@{p1.id}> <@{p2.id}>", embed=emb_m, view=ViewTopico(p1.id, p2.id, med_id, self.valor, self.modo))
...
@bot.command()
async def Pix(ctx):
    emb = discord.Embed(title="Painel Para Configurar Chave PIX", description="Gerencia a tua chave PIX para receberes os pagamentos das mediações.", color=0x2b2d31)
    emb.set_thumbnail(url=bot.user.display_avatar.url)
...
async def canal(ctx):
    ...
    sel.callback = cb; v.add_item(sel); await ctx.send("Escolhe o canal dos tópicos:", view=v)
    ...
    if message.channel.id in partidas_ativas:
        ...
        if message.author.id == dados['med'] and message.content.isdigit():
            ...
            if tid not in temp_dados:
                ...
                await message.channel.send("✅ **ID recebido.** Envia a **Senha** agora.", delete_after=2)
            else:
