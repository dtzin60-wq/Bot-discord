...
class ViewPixMediador(View):
    ...
    async def enviar(self, interaction: discord.Interaction, button: Button):
        ...
        if not cargo_id or not any(r.id == int(cargo_id) for r in interaction.user.roles):
            return await interaction.response.send_message("❌ Apenas o cargo mediador configurado pode enviar o Pix.", ephemeral=True)
        ...
        if not res: return await interaction.response.send_message("❌ Cadastre seu pix com `.pix` primeiro.", ephemeral=True)
...
class ViewConfirmarPartida(View):
    ...
    async def btn_regras(self, interaction: discord.Interaction, button: Button):
        await interaction.response.send_message("📝 Combinem as regras aqui antes do pagamento.", ephemeral=True)
...
class ViewFilaPartida(View):
    ...
    async def entrar(self, interaction, submodo):
        ...
        if len(match) >= 2:
            ...
            # Chama o primeiro mediador da fila suporte
...
async def fila(ctx, valor: str):
    try:
        ...
        emb.add_field(name="Modo", value="`1v1 MOBILE` \n(Gelo Normal ou Infinito)", inline=False)
...
async def pix(ctx):
    class PM(Modal, title="Meu Pix"):
        ...
        async def on_submit(self, it):
            ...
            await it.response.send_message("✅ Pix salvo com sucesso!", ephemeral=True)
    ...
    await ctx.send("Cadastre seus dados para mediação:", view=v)
