...
class MPix(Modal, title="Configurar Chave PIX"):
    ...
    c = TextInput(label="Chave PIX", placeholder="CPF, Email ou Aleatória")
    q = TextInput(label="Link do QR Code", placeholder="URL da imagem do QR Code (opcional)", required=False)
...
class VPix(View):
    ...
    async def ver_sua(self, it: discord.Interaction, b):
        ...
        if not r: return await it.response.send_message("❌ Você não tem uma chave cadastrada.", ephemeral=True)
...
class ViewTopico(View):
    ...
    async def c(self, it: discord.Interaction, b):
        if it.user.id not in [self.p1, self.p2]: return await it.response.send_message("❌ Só os jogadores confirmam!", ephemeral=True)
        self.conf.add(it.user.id)
        await it.response.send_message(f"✅ <@{it.user.id}> confirmou!", delete_after=2)
        
        if len(self.conf) == 2:
            await asyncio.sleep(1)
            await it.channel.purge(limit=30) # LIMPEZA AUTOMÁTICA DO CHAT
...
class ViewFila(View):
    ...
    async def entrar(self, it, gelo):
        ...
        if len(self.p) == 2:
            if not fila_mediadores: 
                self.p = []
                return await it.response.send_message("❌ Sem mediadores!", ephemeral=True)
            
            # ROTAÇÃO: O 1º mediar vai para o último lugar
    ...
    async def sair(self, it: discord.Interaction, b):
        user = next((u for u in self.p if u.id == it.user.id), None)
        if user:
            self.p.remove(user)
            await it.response.edit_message(embed=self.gerar_embed())
        else: await it.response.send_message("⚠️ Você não está na fila!", ephemeral=True)
...
async def canal(ctx):
    ...
    async def cb(i): 
        ...
        await i.response.send_message("✅ Canal de tópicos definido!", ephemeral=True)
...
@bot.event
async def on_ready():
    init_db(); bot.add_view(VPix()); bot.add_view(VMed())
